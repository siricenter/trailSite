...
RSpec.describe "boulder_routes/new", :type => :view do
  before(:each) do
    assign(:boulder_route, BoulderRoute.new(
      :name => "MyString",
      :latitude => "9.99",
      :longitude => "9.99",
      :zoom => 1,
  ...
  it "renders new boulder_route form" do
    render

    assert_select "form[action=?][method=?]", boulder_routes_path, "post" do
